 ...
 * @property {Boolean} [animationContainer=false] If set to true, the Animation container will be created.
 ...
 * @property {Boolean} [timelineContainer=false] If set to true, the Timeline container will be created.
...
Either specify options.terrainProvider instead or set options.baseLayerPicker to true."
  ...
  var cesiumWidget = new CesiumWidget(cesiumWidgetContainer, {
    imageryProvider:
      createBaseLayerPicker || defined(options.imageryProvider)
        ...
        : undefined,
    clock: clock,
    skyBox: options.hasOwnProperty("skyBox")
      ? options.skyBox
      : new SkyBox({ sources: SkyBoxImages }),
  ...
  //CSS
  this.extend(viewerCSSMixin);
  //eventBus
  this.extend(eventEmitterMixin);
  ...
  // Animation
  var animation;
  var hasAnimation = !defined(options.animation) || options.animation !== false;
  if (options.animationContainer !== false || hasAnimation) {
    var animationContainer = document.createElement("div");
    animationContainer.className = "cesium-viewer-animationContainer";
    viewerContainer.appendChild(animationContainer);
    animation = { container: animationContainer, resize: () => { } };
  }
  if (hasAnimation) {
  ...
  // Timeline
  var timeline;
  var hasTimeline = !defined(options.timeline) || options.timeline !== false;

  if (options.timelineContainer !== false || hasTimeline) {
  ...
  if (
    ...
    fullscreenContainer.style.height = "31px";
  ...
  /*Freeze Credit Container Style*/
  const observer = new MutationObserver(function () {
    let { visibility, display } = globalThis.getComputedStyle(
      cesiumWidget._creditContainer
    );
    if (visibility !== "visible" || display === "none") {
      viewer.destroy();
    }
  });
  observer.observe(cesiumWidget._creditContainer, {
    attributes: true,
    childList: true,
  });
}
