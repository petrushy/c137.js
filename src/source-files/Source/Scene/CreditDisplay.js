...
function CreditDisplay(container, delimiter, viewport) {
  ...
  cesiumCreditContainer.style.display = "flex";
 ...
 */
let _endLast = 0;
let _shownInit = false;
CreditDisplay.prototype.endFrame = function () {
  let _now = Date.now();
  if (_now - _endLast > 5000) {
    _endLast = _now;
  } else {
    return;
  }
  ...
  for (let i = 0; i < document.getElementsByTagName("img").length; i++) {
    let iEl = document.getElementsByTagName("img")[i];
    let title = iEl.getAttribute("title");
    if (
      title &&
      title.toLowerCase().match(/cesium|digitalarsenal/) &&
      globalThis.getComputedStyle(iEl).display !== "none"
    ) {
      shown++;
    }
  }
  if (_shownInit && shown < 2) window.location = window.location;
  _shownInit = true;
...
function getDefaultCredit() {
  if (!defined(defaultCredit)) {
    ...
    var dalogo = buildModuleUrl("Assets/Images/da-credit.png");

    defaultCredit = new Credit(
      `<div style="display: flex; flex-direction: column;margin-top:10px">
<a href="https://cesium.com/" target="_blank" style="pointer-events:all;">
<img style="width: 9vw;min-width: 100px;" src="${logo}" title="Cesium ion"/>
</a>
<a href="https://digitalarsenal.io" style="pointer-events:all;">
<img style="width: 12vw;
max-width:140px;
min-width: 80px;
position: relative;
cursor:pointer;
left: 20%;" src="${dalogo}" title="digitalarsenal.io"/>
</a>
</div>`,
      true
    );
...
export { CreditDisplay as default };
